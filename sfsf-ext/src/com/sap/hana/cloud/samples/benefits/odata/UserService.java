...
public class UserService extends ODataService {
	...
	public List<UserInfo> getInfoProfile() throws AppODataException {
		...
		try {
			userInfo = SFODataAPIConnector.getInstance().getUserInfoProfile(currentUser.getUserId());
			users.add(userInfo);
			if (currentUser.getHrManager() != null) {
				hrInfo = SFODataAPIConnector.getInstance().getUserInfoProfile(currentUser.getHrManager().getUserId());
				users.add(hrInfo);
			}
	...
	private String getUserPhoto(String userId, int photoType) throws AppODataException {
		try {
			return SFODataAPIConnector.getInstance().getUserPhoto(userId, photoType);
		} catch (IOException | InvalidResponseException ex) {
