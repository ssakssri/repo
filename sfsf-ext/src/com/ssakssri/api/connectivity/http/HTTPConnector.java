...
import com.ssakssri.api.connectivity.http.headers.BasicAuthenticationHeaderProvider;
import com.ssakssri.api.connectivity.http.headers.ProxyUserHeaderProvider;
...
public class HTTPConnector {
	...
	public SimpleHttpResponse executePOST(String path, String data, String contentType)
	...
	private SimpleHttpResponse executePOST(HttpURLConnection connection, String data, String contentType)
			throws IOException, InvalidResponseException {
		...
		SimpleHttpResponse httpResponse = new SimpleHttpResponse(connection.getURL().toString(), responseCode,
	...
	private void logResponse(SimpleHttpResponse httpResponse) {
		logger.debug("Response from requesting {} is {} {}", httpResponse.getRequestPath(),
				httpResponse.getResponseCode(), httpResponse.getResponseMessage());
		logger.debug("Response content type from requesting {} is {}", httpResponse.getRequestPath(),
				httpResponse.getContentType());
		logger.debug("Response content from requesting {} is {}", httpResponse.getRequestPath(),
	...
	private void injectAuthenticationHeaders(HttpURLConnection urlConnection,
		...
		authenticationHeaders.add(this.proxyUserHeaderProvider.createMappingHeader()); // User Mapping Header required
																						// for Mock API Endpoint
		for (AuthenticationHeader authenticationHeader : authenticationHeaders) {
			...
			logger.info("Auth Header Name : " + authenticationHeader.getName() + " / Value : "
	...
	private URL getRequestURL(DestinationConfiguration destinationConfiguration, String path) throws IOException {
		...
		logger.info("HTTP Request from destination {} with base URL {} and relative path {} resolved to {}",
				this.destinationName, requestBaseURL, path, fullURL.toString());
		return fullURL;
	}
	...
	private synchronized AuthenticationHeaderProvider lookupAuthenticationHeaderProvider() throws IOException {
		try {
			if (this.localAuthenticationHeaderProvider == null) {
				...
				this.localAuthenticationHeaderProvider = (AuthenticationHeaderProvider) ctx
	...
	private DestinationConfiguration lookupDestinationConfiguration() throws IOException {
		...
		if (destinationConfiguration == null) {
			String errorMessage = String.format(
					"Destination %s is not found. Make sure to have the destination configured.", this.destinationName);
			logger.error(errorMessage);
			throw new IOException(errorMessage);
		}
	...
	private synchronized ConnectivityConfiguration lookupConnectivityConfiguration() throws IOException {
		try {
			if (this.localConnectivityConfiguration == null) {
				Context ctx = new InitialContext();
				this.localConnectivityConfiguration = (ConnectivityConfiguration) ctx
	...
	private HttpURLConnection getConnection(String path) throws IOException {
		...
		System.out.println(destinationConfiguration.getAllProperties().toString());
